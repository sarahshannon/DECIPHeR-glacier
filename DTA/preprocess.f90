...
! Temperature
    ...
    if (len_trim(hru_class(6)%filename).gt.0) then
        ...
        PRINT *, 'Subsetting Temperature Grid'
        write(fn_log,*) 'Subsetting Temperature Grid for ', size(gauges_all, 1), ' gauges'
    ...
    if (len_trim(hru_class(8)%filename).gt.0) then
        ...
        do i = 1, size(gauges_all, 1)
            ...
            catch_fn = trim(outputfolder)//'/'//trim(gauge)//trim(hru_class(8)%fn_end)//'.asc'
            call write_ascii_grid(catch_fn, catch_data, catch_ncols, catch_nrows, catch_xllcorner, catch_yllcorner, &
                catch_cellsize, real(-9999, 8), 4)
