program calculate_hrus
    ...
    if ( glacier_switch ) then
        !--- Read in glacier structure first
        temp_filename = trim(outputfolder)//trim(gauge)//'_modelstruct.asc'   
        call read_ascii_grid(temp_filename, hru_class(7)%data, ncols, nrows, xllcorner, yllcorner, cellsize, nodata)
        call reclass_grid(int(hru_class(7)%data), int(nodata), hru_class(7)%class_array, hru_class(7)%meta)  
    end if
        ...
        do k = 1, 8
            ...
            if (len_trim(hru_class(k)%fn_end).gt.0) then
                ...
                if (hru_class(k)%class_type(1:4).eq.'TOPO') then
                    ...
                    !---Class glaciers by elevation only by setting AREA_ID and 
                    !---SLOPE_ID to 1.   
                    if ( glacier_switch ) then                    
                        if (hru_class(k)%fn_end.eq.'_area'.or.hru_class(k)%fn_end.eq.'_slope') then
                            where (hru_class(7)%class_array.eq.2) hru_class(k)%class_array = 1   
                        end if
                    end if
                    !----------------------------------------------------------------------------------
