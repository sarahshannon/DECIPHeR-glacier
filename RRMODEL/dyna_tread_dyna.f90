...
contains
  ...
  subroutine tread_dyna (nac, &
    ...
    do i = 1, num_cols
       ...
       case ('TEMP_ID')
          do j = 1, nac
             dyna_hru(j)%itemp = int(hru_meta(j,i))
          end do
       case ('PARAM_ID')
          do j = 1, nac
    ...
    write(999,*) maxval(dyna_hru%itemp), 'Temperature Grid IDs Specified'
    ...
    !--- sarah read in initial glacier thickness if filename is 
    !--- provided and the file is open    
    inquire(unit=504, opened=itsopen) 
        if ( itsopen ) then
            do ia = 1, nac
                read (504, *) tmp, dyna_hru(ia)%init_glacierdepth
            end do
            write(999,*) 'Initial glacier depth successfully read in'
        end if
      
    close(13)
    close(17)
    close(504)
  end subroutine tread_dyna
